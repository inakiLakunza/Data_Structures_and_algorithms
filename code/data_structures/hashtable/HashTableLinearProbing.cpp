...
template <typename K, typename V>
class HashTableLinearProbing : public HashTableOpenAddressingBase<K, V> {
private:
    static constexpr int LINEAR_CONSTANT = 17;

public:
    HashTableLinearProbing() : HashTableOpenAddressingBase<K, V>() {}

    HashTableLinearProbing(int capacity) : HashTableOpenAddressingBase<K, V>(capacity) {}

    HashTableLinearProbing(int capacity, double loadFactor) : HashTableOpenAddressingBase<K, V>(capacity, loadFactor) {}
    ...
    void adjustCapacity() override {
        while (this->gcd(LINEAR_CONSTANT, this->capacity) != 1) {
            this->capacity++;
        }
    }
};
